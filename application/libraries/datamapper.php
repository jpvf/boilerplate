...
class DataMapper implements IteratorAggregate {
	...
	public function DataMapper($id = NULL)
	{
		...
		if ( ! isset(DataMapper::$common[$common_key]))
		{
			...
			if ($is_dmz)
			{
				...
				Config::load('datamapper', TRUE, TRUE);

				// Get and store config settings
				DataMapper::$config = Config::get_group('datamapper');
	...
	public static function autoload($class)
	{
		...
		if (in_array(substr($class, 0, 3), array('CI_', 'EE_')) OR strpos($class, Config::get('subclass_prefix')) === 0)
		{
			return;
		}
		...
		Loader::getInstance()->model($class);
	}
	...
	protected static function _load_extensions(&$extensions, $names)
	{
		...
		foreach($names as $name => $options)
		{
			...
			{
				$file = RUTA_APP . DataMapper::$config['extensions_path'] . '/' . $name . EXT;
				$ext = $name;
			}
			else
			{
				$file = RUTA_APP . $name . EXT;
				$ext = array_pop(explode('/', $name));
			}

			if(!file_exists($file))
			{
				 echo 'DataMapper Error: loading extension ' . $name . ': File not found.';
			}
